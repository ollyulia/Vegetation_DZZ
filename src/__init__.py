    ...
    def download_images_by_coordinates(
        ...
        def sendRequest(url, data, api_key=None):
            ...
            try:
                ...
                if output["errorCode"] is not None:
                    print("Неудачный запрос. ID: ", out-put["requestId"])
                    print(output["errorCode"], "-", out-put["errorMessage"])
                    sys.exit()
        ...
        def downloadFile(url):
            ...
            try:
                ...
                filename = re.findall("filename=(.+)", disposi-tion)[0].strip("\"")
        ...
        print("Авторизация...")
        apiKey = sendRequest(serviceUrl + "login-token", log-in_payload)
        print("API ключ: успех")

        datasetName = "landsat_ot_c2_l2" # gls_all / land-sat_ot_c2_l2 / land-sat_ot_c2_l1 / landsat_band_files_c2_l1 / lsr_nadsat_etm_c1 /
        ...
        if scenes["recordsReturned"] > 0:
            print(f"Найдено {scenes["recordsReturned"]} подходя-щих сцен")
            ...
            for product in products:
                ...
                if product['available']:
                    if product["secondaryDownloads"] is not None and len(product["secondaryDownloads"]) > 0:
                        for secondaryDownload in prod-uct["secondaryDownloads"]:
                            for bandName in bandNames:
                                if secondaryDown-load["bulkAvailable"] and bandName in secondaryDown-load["displayId"]:
                                    downloads.append(
                                        {
                                            "entityId": secondaryDown-load["entityId"],
                                            "productId": secondaryDown-load["id"]
                                        }
                                    )
            ...
            if downloads:
                ...
                requestResults = sendRequest(serviceUrl + "down-load-request", download_req_payload, apiKey)

                print(f"Обнаружено подходящих файлов для скачива-ния {len(requestResults["availableDownloads"])}")

                # dump_json(requestResults, "3_request_results")

                # В PreparingDownloads есть действующая ссылка, которую можно использовать, но данные могут быть доступны не сра-зу
                # Вызовите метод download-retrieve, чтобы полу-чить загрузку, доступную для немедленного скачивания
                if requestResults['preparingDownloads'] is not None and len(requestResults['preparingDownloads']) > 0:
                    ...
                    for download in moreDownloadUrls['available']:
                        if str(download['downloadId']) in reques-tRe-sults['newRecords'] or str(download['downloadId']) in \
                    ...
                    # Если вы не получили все повторно загружен-ные файлы, вы-зовите метод загрузки-извлечения еще раз, через 30 секунд
                    while len(downloadIds) < (requestedDown-loadsCount - len(requestResults['failed'])):
                        preparingDownloads = requestedDown-loadsCount - len(downloadIds) - len(requestResults['failed'])
                        print("\n", preparingDownloads, "загрузок недоступно. Следующая попытка через 30 секунд...")
                        time.sleep(30)
                        print("Попытка получить данные")
                        moreDownloadUrls = sendRequest(serviceUrl + "download-retrieve", payload, apiKey)
                        for download in moreDown-loadUrls['available']:
                            if download['downloadId'] not in downloadIds and (
                                    str(download['downloadId']) in requestRe-sults['newRecords'] or str(
                                    download['downloadId']) in requestRe-sults['duplicateProducts']):
                                downloadIds.append(download['downloadId'])
                                runDownload(threads, download['url'])

                else:
                    # Get all available downloads
                    for download in requestRe-sults['availableDownloads']:
                        runDownload(threads, download['url'])
        else:
        ...
        # Выйдите из системы, чтобы больше нельзя было использо-вать API-ключ
        endpoint = "logout"
